<?php
	...
	if ($account->tel == '') {
	$q = 'select * from proveedores where nombre="'.$account->name.'";';
	}
	if($account->name == '') {
	$q = 'select * from proveedores where telefono='.$account->tel;
	}
	if($account->name != '' && $account->tel != '') {
	$q = 'select * from proveedores where telefono='.$account->tel.' AND nombre="'.$account->name.'";';
	}
	$qy = $database->q($con, $q);
	if ($qy) {
		$results = array();
